% 计算树的z坐标程序
function tree_pos = leaf_position_tree_pos(sn,pos,bc) %坡面的法向量 树冠中心在样地中的xyz坐标 正方形样方边长(垂直视)

% 坡面中心过(0,0,0)时样地4角点的xyz坐标
pCorner=[[ bc/2, bc/2,-(sn(1)*( bc/2) +sn(2)*( bc/2))/sn(3)];...
         [-bc/2, bc/2,-(sn(1)*(-bc/2) +sn(2)*( bc/2))/sn(3)];...
         [ bc/2,-bc/2,-(sn(1)*( bc/2) +sn(2)*(-bc/2))/sn(3)];...
         [-bc/2,-bc/2,-(sn(1)*(-bc/2) +sn(2)*(-bc/2))/sn(3)]];
