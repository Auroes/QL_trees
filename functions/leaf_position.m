...
%% 读入参数矩阵
tmp=strcat('.\forestModel\',canopy_para); % 构建文件路径字符串tmp，将文件路径指定为当前目录下的forestModel文件夹，附加canopy_para作为文件名
...
center = [];
for i=1:size_crown(2) % 循环每个树冠 2是列的大小
    ...
    z = r.*cos(angle2); % 生成每棵树冠中叶片的随机位置 包括随机的角度angle1和angle2 以及计算叶片的三维坐标
    %     figure
    %     plot3(x,y,z,'r.');
    %     axis square

    pos = [canopy_para(2,i),canopy_para(3,i),0]; % 读入的第i棵树树冠中心在样地中的xyz坐标
    tree_pos = leaf_position_tree_pos(sn,pos,edge); % 每棵树树根在坡面上的xyz坐标
    pCorner = squareCorner(sn,edge);% 坡面中心过(0,0,0)时样地4角点的xyz坐标
